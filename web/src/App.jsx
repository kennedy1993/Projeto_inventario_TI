...
function App() {
  ...
  const exportToExcel = () => {
    ...
    if (isToner) {
      ...
      const colW = [16, 12, 12, 15, 14, 20, 35, 22, 18, 14, 12, 22, 18, 14];
      if (hasChip) colW.splice(3, 0, 18);
      ws['!cols'] = colW.map(w => ({ wch: w }));
    }
  ...
  const exportToPDF = () => {
    ...
    if (isToner) {
      ...
      tableColumn = hasChip
        ? ['TAG', 'Qtd', 'Tipo', 'Nº Chip', 'Equipamento', 'Colaborador', 'Setor', 'Local', 'Status', 'Valor (R$)']
        : ['TAG', 'Qtd', 'Tipo', 'Equipamento', 'Colaborador', 'Setor', 'Local', 'Status', 'Valor (R$)'];
      tableRows = filteredAtivos.map(a => {
        const base = [
          a.tag_patrimonio,
          a.quantidade || 1,
        ...
        if (hasChip) base.splice(3, 0, a.tipo === 'CELULAR' ? (a.numero_chip || '—') : '—');
        return base;
      });
    }
                ...
                <>
                  ...
                  <div className="stat-card" style={{flex: 1, padding: '1rem'}}>
                    ...
                    <div className="stat-value" style={{fontSize: '1.25rem', color: 'var(--success)'}}>
                      R$ {filteredAtivos.reduce((acc, curr) => acc + ((curr.valor || 0) * (curr.quantidade || 1)), 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </div>
                  </div>
                </>
              ) : (
